...
public class BST {
	...
	public int replaceWithSumLarger(Node node, int sum) {
		...
		int sumOfLargerValues = replaceWithSumLarger(node.right, sum);

		int temp = node.data;
		node.data = sumOfLargerValues;

		int leftIncludeLargerValuesSum = replaceWithSumLarger(node.left, sumOfLargerValues + temp);

		return leftIncludeLargerValuesSum;
